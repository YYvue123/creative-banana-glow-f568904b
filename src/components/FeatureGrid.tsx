...
const handleTryNow = async () => {
  ...
  // Navigate user to the generation page or scroll to top
};
  ...
  if (state === 'loading') {
    return (
      <section className="container mx-auto space-y-20 px-4 py-20">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="flex flex-col items-center gap-10 md:flex-row md:gap-16">
            <div className="flex-1 space-y-4">
              <div className="h-8 w-3/4 animate-pulse rounded-lg bg-muted" />
              <div className="h-20 w-full animate-pulse rounded-lg bg-muted" />
            </div>
            <div className="flex-1">
              <div className="aspect-video w-full animate-pulse rounded-[2rem] bg-muted" />
            </div>
          </div>
        ))}
      </section>
          ...
          >
            ...
            <div>
              <button
                ...
                className="rounded-full bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-80"
              >
